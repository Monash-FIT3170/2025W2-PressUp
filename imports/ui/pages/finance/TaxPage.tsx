...
import { format, startOfMonth, startOfYear, endOfMonth, endOfYear, endOfDay } from "date-fns";
...
export const TaxPage = () => {
  ...
  const [deductionForm, setDeductionForm] = useState({ name: "", date: format(new Date(), "yyyy-MM-dd"), description: "", amount: "" });
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const fetchedDeductions = await Meteor.callAsync("deductions.getAll") as any[];
  ...
  const { start, end } = useMemo(() => {
    if (dateRange === "all") return { start: new Date(0), end: endOfDay(new Date()) };
  ...
  const filteredDeductions = useMemo(() => deductions.filter(d => (d.date instanceof Date ? d.date : new Date(d.date)) >= start && (d.date instanceof Date ? d.date : new Date(d.date)) <= end), [deductions, start, end]);
  ...
  // GST calculations
  const { GSTTotal, GSTItems, GSTExpenses, expenseItems, profitTotal, expensesTotal, deductionsTotal } = useMemo(() => {
    let GSTTotal = 0, profitTotal = 0, GSTExpenses = 0, expensesTotal = 0, deductionsTotal = 0;
    ...
    return {
      GSTTotal,
      GSTItems: Object.entries(GSTByDate).map(([label, amount]) => ({ label, amount })),
      GSTExpenses,
      expenseItems: Object.entries(expensesByDate).map(([label, amount]) => ({ label, amount })),
      profitTotal,
      expensesTotal,
      deductionsTotal
    };
      ...
      <div className="w-full p-6 bg-gray-50 min-h-[100vh]">
        ...
        <div className="flex items-baseline gap-4 mb-4">
          ...
          <h2 className="ml-4 text-red-900">
            <span className="font-bold">Viewing Period:</span> {getDateRangeText}
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          {mainMetrics.map(metric => (
            <FinanceCard
              ...
              onClick={() => setSelectedMetric(metric.key as "incomeTax" | "payrollTax" | "GSTCollected" | "GSTPaid")}
            />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {(selectedMetric === "GSTCollected" || selectedMetric === "GSTPaid" || selectedMetric === "payrollTax") && (
            <div className="bg-white rounded-xl shadow-lg p-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-1">{currentData.title}</h2>
              <p className="text-gray-600 mb-4">{currentData.description}</p>
              <div className="h-80 w-full">
                <ResponsiveContainer>
                  <BarChart data={selectedMetric === "GSTCollected" || selectedMetric === "GSTPaid" ? combinedItems : currentData.items}>
