...
import {
  ...
  TrainingList,
} from "./TrainingListsCollection";
import {
  TrainingProgressCollection,
  TrainingProgress,
} from "./TrainingProgressCollection";
...
Meteor.methods({
  ...
  "trainingLists.update": requireLoginMethod(async function (
    ...
    const result = await TrainingListsCollection.updateAsync(listId, {
      $set: updates,
    });
    ...
    if (updates.items !== undefined) {
      ...
      const progresses = await TrainingProgressCollection.find({
      ...
      for (const progress of progresses) {
        ...
        for (const itemId of newItemIds) {
          newCompletedItems[itemId] =
            progress.completedItems?.[itemId] ?? false;
        }

        // Update the progress entry
        await TrainingProgressCollection.updateAsync(progress._id, {
  ...
  "trainingProgress.ensureForStaff": requireLoginMethod(async function (
    staffId: string,
    ...
    if (existing) {
      ...
      return await TrainingProgressCollection.updateAsync(existing._id, {
        $set: { completedItems },
      });
    } else {
      // Create new entry
      completedItems = itemIds.reduce(
        (acc, itemId) => {
          acc[itemId] = false;
          return acc;
        },
  ...
  "trainingProgress.update": requireLoginMethod(async function (
    ...
    if (updates.trainingListId !== undefined)
      check(updates.trainingListId, String);
    if (updates.completedItems !== undefined) {
      check(updates.completedItems, Object);
    }
    return await TrainingProgressCollection.updateAsync(progressId, {
      $set: updates,
    });
  }),
  ...
  "trainingProgress.remove": requireLoginMethod(async function (
    progressId: IdType,
  ) {
    check(progressId, String);
