...
Meteor.publish("shifts.current", requireLoginPublish(function() {
...
Meteor.publish("shifts.all", requireLoginPublish(async function() {
...
Meteor.publish("shifts.dateRange", requireLoginPublish(async function(startDate: Date, endDate: Date) {
  if (!await Roles.userIsInRoleAsync(this.userId, [PressUpRole.ADMIN, PressUpRole.MANAGER])) {
    return ShiftsCollection.find({
      user: this.userId,
      $or: [
...
Meteor.publish("shifts.active", requireLoginPublish(async function() {
  if (!await Roles.userIsInRoleAsync(this.userId, [PressUpRole.ADMIN, PressUpRole.MANAGER])) {
    return ShiftsCollection.find({
      user: this.userId,
      end: { $exists: false }
