...
} from "/imports/api/accounts/userTypes";
...
Meteor.methods({
  ...
  "users.updateProfile": requireLoginMethod(async function (
    userId: IdType,
    ...
    try {
      return await Meteor.users.updateAsync(userId, {
        $set: {
          "profile.firstName":
            profileData.firstName || userToUpdate.profile?.firstName,
          "profile.lastName":
            profileData.lastName || userToUpdate.profile?.lastName,
        },
      });
    } catch (error) {
      const errorMessage =
  ...
  "users.updateRole": requireLoginMethod(async function (
    userId: string,
    newRole: RoleEnum,
  ) {
    check(userId, String);
    ...
    if (
      userId === this.userId &&
      !(await Roles.userIsInRoleAsync(this.userId, [RoleEnum.ADMIN]))
    ) {
      throw new Meteor.Error(
    ...
    try {
      ...
      console.log(
        `Updated user '${userToUpdate.username}' role to '${newRole}'`,
      );
      return true;
    } catch (error) {
      const errorMessage =
